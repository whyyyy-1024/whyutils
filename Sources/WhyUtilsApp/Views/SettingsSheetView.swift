...
struct SettingsSheetView: View {
    ...
    private var aiSection: some View {
        ...
        return settingsCard(title: coordinator.localized("AI Assistant", "AI 助手")) {
            ...
            if accessMode.wrappedValue == .fullAccess {
                Text(coordinator.localized(
                    "Full Access mode exposes shell execution and direct file operations to the model. Only enable it for trusted providers and keys.",
                    "Full Access 模式会向模型开放 shell 执行和直接文件操作。仅在你信任当前模型供应商与密钥时开启。"
                ))
                .font(.system(size: 12, weight: .medium))
    ...
    private func accessModeDescription(_ mode: AIAgentAccessMode) -> String {
        ...
        case .fullAccess:
            return coordinator.localized(
                "Full Access allows shell, file, and URL tools without confirmation. Plans can expand up to 8 steps.",
                "Full Access 开放 shell、文件和 URL 工具，无需二次确认。执行计划最多 8 步。"
            )
        }
    }
}
